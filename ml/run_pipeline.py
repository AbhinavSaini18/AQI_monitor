...
def run_predictions():
    ...
    if latest_snapshots is None or latest_snapshots.empty:
        print("Data ingestion empty. Halting loop.")
        return
    ...
    m24 = xgb.XGBRegressor(); m24.load_model("models/xgboost_aqi_24h.json")
    m48 = xgb.XGBRegressor(); m48.load_model("models/xgboost_aqi_48h.json")
    m72 = xgb.XGBRegressor(); m72.load_model("models/xgboost_aqi_72h.json")
    ...
    for idx, row in latest_snapshots.iterrows():
        ...
        t_sc = max(5.0, float(row['NO2']) * 1.2)
        i_sc = max(5.0, float(row['SO2']) * 2.5)
